...
for leftInFile in *_1.fastq.gz
	do
		...
		$leftInFile \
		$rightInFile \
		$pairedOutPath$leftInFile.paired.fastq.gz \
		$unpairedOutPath$leftInFile.unpaired.fastq.gz \
		$pairedOutPath$rightInFile.paired.fastq.gz \
		$unpairedOutPath$rightInFile.unpaired.fastq.gz \
		HEADCROP:0 \
		ILLUMINACLIP:/usr/local/programs/Trimmomatic-0.36/adapters/NexteraPE.fa:2:30:10 \
